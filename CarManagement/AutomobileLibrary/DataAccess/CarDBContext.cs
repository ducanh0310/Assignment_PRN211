...
namespace AutomobileLibrary.DataAccess
{
    public class CarDBContext : BaseDAL
    {
        ...
        public IEnumerable<Car> FilterName(string name)
        {
            var list = (List<Car>)this.GetCarLists();

            var newList = list.FindAll(member => member.CarName.ToLower().Contains(name.ToLower()));
            return newList;
        }


        //----------------------------------------------------------
        public IEnumerable<Car> FilterManufacture(string manu)
        {
            var list = (List<Car>)this.GetCarLists();

            var newListByCity = list.FindAll(member => member.Manufacturer.ToLower().Equals(manu.ToLower()));
